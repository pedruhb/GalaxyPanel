										...
										<div class="tab-pane active" role="tabpanel">
											<div class="card">
												<div class="card-header">
													<h5 class="card-header-text">Reinstalar câmera</h5>
													<span>Reinstale sua câmera, isso irá apagar todas as fotos e thumbnails de quarto.</span>
												</div>
														...
														if(!permissao("recms")){
															...
															if (isset($_POST['reinstalar']))
															{
																...
																if($_POST['senha'] != $server['senhavestacp']){
																	...
																	ob_start(); 
																	set_time_limit(0); 
																	$sourcedir="galaxyservers/galaxycms/cms3/camera/"; 
																	$ftpserver=$iphospedagem; 
																	$ftpusername=$server['uservestacp'];  
																	$ftppass=$server['senhavestacp']; 
																	$ftpremotedir= 'web/'.$linkhotel.'/public_html/camera'; 
																	...
																	if (ftp_nlist($ftpconnect, './web/'.$linkhotel.'/public_html/camera/') == true) {
																		ftp_rdel($ftpconnect, './web/'.$linkhotel.'/public_html/camera/');
																	}
																	ftp_mkdir($ftpconnect, '/web/'.$linkhotel.'/public_html/camera/');
																	...
																	<strong>Sucesso!</strong> Câmera reinstalada.
																	</div>';

																	addlog('Reinstalou a câmera.');

																	$PDO2 = new PDO('mysql:host='.$iphospedagem.';dbname='.$server['uservestacp']."_hotel", $server['uservestacp']."_hotel", $server['senhavestacp']);								
																	$excluitabelas = $PDO2->prepare("DROP TABLE IF EXISTS server_pictures;");
																	$excluitabelas->execute();
																	$excluitabelas2 = $PDO2->prepare("DROP TABLE IF EXISTS server_pictures_publish;");
																	$excluitabelas2->execute();
																	$inseretabelas = $PDO2->prepare("CREATE TABLE `server_pictures` (
																		...
																		AUTO_INCREMENT=60
																		;
																		CREATE TABLE `server_pictures_publish` (
																		`id` INT(11) NOT NULL AUTO_INCREMENT,
															...
															<center>
																<form method="POST">
																	...
																	<button type="submit" name="reinstalar" style="border-radius: 8px;" class="btn btn-primary btn-square" onclick="return confirm('Tem certeza que deseja reinstalar a câmera? você irá perder todas as fotos e miniaturas de quarto!')">Reinstalar</button>
																</form>
															</center>
														</div>
														</div><?php }?>
