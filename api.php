<?php
#### API EM PHP PARA O GALAXYPANEL BY PHB.
set_time_limit(0);
...
function rconAPI($command, $data, $ipvp2s, $mus) {
...
if($funcao == "ligaremulador"){
$usuario = $_POST['usuario'];
$senha = $_POST['senha'];

$pegatotal1 = $PDO->prepare("SELECT * FROM servidores WHERE uservestacp = :usuario AND senhavestacp = :senhavesta AND ativo = 'Y';");
$pegatotal1->bindParam(':usuario', $usuario);
$pegatotal1->bindParam(':senhavesta', $senha);
$pegatotal1->execute();
if($pegatotal1->rowCount() == 0)
die("Não há nenhum hotel ativo com esses dados.");

$hotel = $pegatotal1->fetch();

if($hotel['estado'] == "ligado")
die('O emulador já está ligado!');
else {
popen("C:/xampp/htdocs/galaxyservers/emuladores/".$hotel['uservestacp']."/galaxyserverligar.bat", "r");

$atualizastatus = $PDO->prepare("UPDATE servidores SET estado = 'ligado' WHERE portamus = ".$hotel['portamus']);
$atualizastatus->execute();

$adicionalog = $PDO->prepare("INSERT INTO `log_servidores` (`log`, `autor`, `ip`, `data`) VALUES ('Ligou o emulador através da API.', '".$hotel['uservestacp']."', '', '".time()."');");
$adicionalog->execute();
echo 'success';
}
}
else if($funcao == "desligaremulador"){
$usuario = $_POST['usuario'];
$senha = $_POST['senha'];
...
if($hotel['estado'] == "desligado")
die('O emulador já está desligado!');
else {
rconAPI('desligaremulador', '', $ipvps, $hotel['portamus']);

$atualizastatus = $PDO->prepare("UPDATE servidores SET estado = 'desligado' WHERE portamus = '".$hotel['portamus']."'");
$atualizastatus->execute();

$adicionalog = $PDO->prepare("INSERT INTO `log_servidores` (`log`, `autor`, `ip`, `data`) VALUES ('Desligou o emulador através da API.', '".$hotel['uservestacp']."', '', '".time()."');");
$adicionalog->execute();
echo 'success';
}
}
else if($funcao == "reiniciaremulador"){
$usuario = $_POST['usuario'];
$senha = $_POST['senha'];
...
if($hotel['estado'] == "desligado")
die('O emulador está desligado!');
else {
rconAPI('reiniciaremulador', '', $ipvps, $hotel['portamus']);

$adicionalog = $PDO->prepare("INSERT INTO `log_servidores` (`log`, `autor`, `ip`, `data`) VALUES ('Reiniciou o emulador através da API.', '".$hotel['uservestacp']."', '', '".time()."');");
$adicionalog->execute();
echo 'success';
}
}
...
if(empty($adminpass))
die("Senha admin vazia.");
if(empty($adminusuario))
die("Usuário admin vazio.");

$contauser = $PDO->prepare("SELECT id FROM admin where login = :usuario and senha = :senha");
$contauser->bindParam(':usuario', $adminusuario);
$contauser->bindParam(':senha', $adminpass);
$contauser->execute();
if($contauser->rowCount() == 0)
...
$pegatotal1 = $PDO->prepare("SELECT * FROM servidores WHERE uservestacp = :usuario");
$pegatotal1->bindParam(':usuario', $usuario);
$pegatotal1->execute();
if($pegatotal1->rowCount() == 0)
die("Não há nenhum hotel com esses dados.");

$hotel = $pegatotal1->fetch();
rconAPI('desligaremulador', '', $ipvps, $hotel['portamus']);

if($hotel['senhavestacp'] == $novasenha)
die("A senha é a mesma da atual!");

$novaApiKey = md5($hotel['uservestacp']."galaxyservers".$novasenha);

$attSenha = $PDO->prepare("UPDATE servidores SET senhavestacp = :senha, apikey = :api WHERE uservestacp = :usuario");
$attSenha->bindParam(':senha', $novasenha);
$attSenha->bindParam(':api', $novaApiKey);
$attSenha->bindParam(':usuario', $usuario);
$attSenha->execute();
...
$adicionalog = $PDO->prepare("INSERT INTO `log_servidores` (`log`, `autor`, `ip`, `data`) VALUES ('Mudou a senha através da API.', '".$hotel['uservestacp']."', '', '".time()."');");
$adicionalog->execute();
...
db.port=3306
db.username=".$hotel['uservestacp']."_hotel
db.password=".$hotel['senhavestacp']."
db.name=".$hotel['uservestacp']."_hotel
...
hotel.name=".$hotel['nomehotel']."
license=".$hotel['nomehotel']." Hotel
...
tipohall=2";
$CaminhoConfig = "galaxyservers/emuladores/".$hotel['uservestacp']."/Config/config.ini";
...
$pegatotal1 = $PDO->prepare("SELECT id,ativo,portamus FROM servidores WHERE uservestacp = :usuario");
$pegatotal1->bindParam(':usuario', $usuario);
$pegatotal1->execute();
if($pegatotal1->rowCount() == 0)
die("Não há nenhum hotel com esses dados.");
...
$apagaref1 = $PDO->prepare("UPDATE servidores SET ativo='Y' where uservestacp = :uservestacp ");
$apagaref1->bindParam(':uservestacp', $usuario);
$apagaref1->execute();

$log = 'Apagou o hotel através da API "'.$usuario.'".';
$adicionalog = $PDO->prepare("INSERT INTO `log_admin` (`log`, `autor`, `ip`, `data`) VALUES (:log, :admin, '', '".time()."');");
$adicionalog->bindParam(':log', $log);
$adicionalog->bindParam(':admin', $adminusuario);
$adicionalog->execute();
...
$pegatotal1 = $PDO->prepare("DELETE FROM servidores WHERE uservestacp = :usuario");
$pegatotal1->bindParam(':usuario', $usuario);
$pegatotal1->execute();

$pegatotal1 = $PDO->prepare("DELETE FROM emblemas WHERE usuario = :usuario");
$pegatotal1->bindParam(':usuario', $usuario);
$pegatotal1->execute();

$pegatotal1 = $PDO->prepare("DELETE FROM log_servidores WHERE autor = :usuario");
$pegatotal1->bindParam(':usuario', $usuario);
$pegatotal1->execute();

$ApagaConfig = $PDO->prepare("DELETE FROM config_servidores WHERE servidor = :user");
$ApagaConfig->bindParam(':user', $idhp);
$ApagaConfig->execute();
...
$ApagaSubcontas->bindParam(':user', $idhp);
...
delTree('galaxyservers/emuladores/'.$usuario.'/');

echo 'OK'; 
return;
} else if($funcao == "reativarhabbo"){
$adminusuario = $_POST['adminusuario'];
$adminpass = md5($_POST['adminpass']);
$usuario = $_POST['usuario'];
...
$pegatotal1 = $PDO->prepare("SELECT id,senhavestacp FROM servidores WHERE uservestacp = :usuario");
$pegatotal1->bindParam(':usuario', $usuario);
$pegatotal1->execute();
if($pegatotal1->rowCount() == 0)
die("Não há nenhum hotel com esses dados.");

$senhaservidor = $pegatotal1->fetch()['senhavestacp'];

$apagaref1 = $PDO->prepare("UPDATE servidores SET ativo='Y' where uservestacp = :usuario");
$apagaref1->bindParam(':usuario', $usuario);
$apagaref1->execute();


$log='Ativou o hotel através da API "'.$usuario.'"';
$adicionalog = $PDO->prepare("INSERT INTO `log_admin` (`log`, `autor`, `ip`, `data`) VALUES (:log, :usuarioadm, '', '".time()."');");
$adicionalog->bindParam(':log', $log);
$adicionalog->bindParam(':usuarioadm', $adminusuario);
$adicionalog->execute();
...
$pegatotal1 = $PDO->prepare("SELECT id,portamus FROM servidores WHERE uservestacp = :usuario");
$pegatotal1->bindParam(':usuario', $usuario);
$pegatotal1->execute();
if($pegatotal1->rowCount() == 0)
die("Não há nenhum hotel com esses dados.");
...
$apagaref1 = $PDO->prepare("UPDATE servidores SET ativo='N' where uservestacp = :usuario");
$apagaref1->bindParam(':usuario', $usuario);
$apagaref1->execute();

rconAPI('desligaremulador', '', $ipvps, $hotel['portamus']);

$log='Suspendeu o hotel através da API "'.$usuario.'"';
$adicionalog = $PDO->prepare("INSERT INTO `log_admin` (`log`, `autor`, `ip`, `data`) VALUES (:log, :usuarioadm, '', '".time()."');");
$adicionalog->bindParam(':log', $log);
$adicionalog->bindParam(':usuarioadm', $adminusuario);
$adicionalog->execute();
...
$pegatotal1 = $PDO->prepare("SELECT id,portamus,plano FROM servidores WHERE uservestacp = :usuario");
$pegatotal1->bindParam(':usuario', $usuario);
$pegatotal1->execute();
if($pegatotal1->rowCount() == 0)
die("Não há nenhum hotel com esses dados.");
...
if($hotel['plano'] == $pacote)
die("O novo pacote informado é o mesmo do atual.");

$pegadados = $PDO->prepare("SELECT vesta_package FROM planos WHERE id = :id");
$pegadados->bindParam(':id', $pacote);
$pegadados->execute();
...
$apagaref1 = $PDO->prepare("UPDATE servidores SET plano = :pacote where uservestacp = :usuario");
$apagaref1->bindParam(':usuario', $usuario);
$apagaref1->bindParam(':pacote', $pacote);
$apagaref1->execute();

$log='Alterou o pacote do "'.$usuario.'" para o plano '.$pacote;
$adicionalog = $PDO->prepare("INSERT INTO `log_admin` (`log`, `autor`, `ip`, `data`) VALUES (:log, :usuarioadm, '', '".time()."');");
$adicionalog->bindParam(':log', $log);
$adicionalog->bindParam(':usuarioadm', $adminusuario);
$adicionalog->execute();
...
else die("Erro ao mudar pacote do hotel!");
}
return;
} else if($funcao == "novohabbo"){

$nomehotel = str_replace(" Hotel", "", str_replace(" hotel", "", $_POST['nomehotel']));
$linkhotel = strtolower(str_replace("..", ".", str_replace(" ", "",str_replace("http", "", str_replace("https", "", str_replace(":", "", str_replace("/", "", $_POST['linkhotel'])))))));
$linkdohotel = $linkhotel;
$usuario = $_POST['usuario'];
$senha = $_POST['senha'];
$cms = "cms1";
$email = $_POST['email'];
...
die("Senha admin vazia.");
if(empty($adminusuario))
die("Usuário admin vazio. ".$adminusuario);
...
if(empty($nomehotel))
die("Nome do hotel vazio.");
if(empty($linkhotel))
die("Link do hotel vazio.");
if(empty($usuario))
die("Usuário vazio.");
if(empty($senha))
die("Senha vazia.");
...
die("HTTP vazio.");
if(strlen($usuario) >= 9)
die("O usuário deve ter no máximo 8 caracteres.");

$likedominio1 = $linkhotel.'%';
$likedominio2 = 'https://'.$linkhotel.'%';
$likedominio3 = 'http://'.$linkhotel.'%';
...
$containserts->bindParam(":uservestacp", $usuario);
$containserts->execute();
$conta = $containserts->fetch();
if($conta['totalinsert'] > 0)
die("Já existe um hotel com esse nome de usuário! 1");
...
$conta = $containserts->fetch();
if($conta['totalinsert'] > 0)
die("Já existe um hotel com esse nome de usuário! 2");

$timenow = time();
$apikeygalaxy = md5($usuario."galaxyservers".$senha);
...
$porta_tcp = $porta['tcpnovo'];
$porta_mus = $porta['musnovo'];


$getversao = $PDO->prepare("SELECT versao FROM emulador;");
$getversao->execute();
$versao = $getversao->fetch()['versao'];

$getversao2 = $PDO->prepare("SELECT id FROM att_db ORDER BY id desc LIMIT 1");
$getversao2->execute();
$versao2 = $getversao2->fetch()['id'];
...
$adicionadb->bindParam(':uservestacp', $usuario);
$adicionadb->bindParam(':nomehotel', $nomehotel);
$adicionadb->bindParam(':linkhotel', $linkdohotel);
$adicionadb->bindParam(':senhavestacp', $senha);
$adicionadb->bindParam(':emailcliente', $email);
$adicionadb->bindParam(':tcp', $porta_tcp);
$adicionadb->bindParam(':mus', $porta_mus);
$adicionadb->bindParam(':data', $timenow);
$adicionadb->bindParam(':apikey',$apikeygalaxy);
$adicionadb->bindParam(':plano', $espaco);
$adicionadb->bindParam(':http', $http);
$adicionadb->bindParam(':vemu', $versao);
$adicionadb->bindValue(':adb', $versao2);
$adicionadb->execute();
} catch (Exception $e) {
...
$insereconfigs->bindParam(":nomehotel", $nomehotel );
$insereconfigs->execute();

$pasta = "galaxyservers/emuladores/sourcegalaxy"; //pasta que sera copiada
$pastad = "galaxyservers/emuladores/".$usuario;//pasta de destino pode ou nÃao estar criada
...
$pastad2 = "galaxyservers/emuladores/".$usuario."/Config";//pasta de destino pode ou nÃo estar criada
...
$fp = fopen("galaxyservers/emuladores/".$usuario."/galaxyserverligar.bat", "w");
$escreve = fwrite($fp, "cd C:/xampp/htdocs/galaxyservers/emuladores/".$usuario."/
start emukk.exe
exit");

fclose($fp);
$nomearquivo = "galaxyservers/emuladores/".$usuario."/Config/config.ini";
...
db.hostname=".$iphospedagem."
db.port=3306
db.username=".$usuario."_hotel
db.password=".$senha."
db.name=".$usuario."_hotel
...
$nomearquivo1 = "galaxyservers/emuladores/".$usuario."/Config/extras.ini";
...
die('Erro ao criar conta de hospedagem (Erro '.$answer.')');
}
...
die('Erro ao adicionar o domínio '.$linkdohotel.' para o '.$usuario.' (Erro '.$answer.').');
}
...
die('Erro ao criar banco de dados (Erro '.$answer.').');
}



try{
$PDO2 = new PDO('mysql:host='.$iphospedagem.';dbname='.$usuario."_hotel", $usuario."_hotel", $senha);
$importa = $PDO2->prepare(file_get_contents("galaxyservers/emuladores/database_gservers.sql"));
$importa->execute();
}catch(Exception $e){
echo 'Erro ao instalar banco de dados. ('.$e.')';
}
...
pushstring          "'.$http.'://'.$linkdohotel.'/camera/index.php"
getlocal0
getproperty         QName(PrivateNamespace("_-4E0", "_-3XF:CameraPhotoLab#0"), "_-5dD")
...
findpropstrict      QName(PackageNamespace("flash.net"), "URLRequest")
pushstring          "'.$http.'://'.$linkdohotel.'/camera/thumbnails/index.php?roomid="
getlocal            4
...
if ( @ftp_put( $con_id, "./"."'.$usuario.'".".swf", "C:\/xampp\htdocs\galaxyservers\CompilaHabboSwf2\Habbo.swf", FTP_BINARY)) 
{
...
?>';
$GalaxyServersBat = '@title Compilador de SWF 2 - GalaxyServers.
@color a 
@echo Compilador de SWF automatica GalaxyServers.
@echo Desenvolvido por PHB
@echo Compilando a SWF do '.$usuario.' Hotel...
@echo ----------------------------------------------------
...
ftp_login( $con_id, "'.$userftpswf.'", "'.$senhaftpswf.'" );
ftp_pasv($con_id, true) or die("Não foi possível entrar no modo passivo."); 
if ( @ftp_put( $con_id, "./"."'.$usuario.'".".swf", "C:\/xampp\htdocs\galaxyservers\CompilaHabboSwf\Habbo.swf", FTP_BINARY)) 
{
echo "SWF upada com sucesso";
} else {
echo "Erro ao enviar swf.";
...
@rabcasm Habbo-2/Habbo-2.main.asasm
@echo off
C:\xampp\php\php.exe C:\xampp\htdocs\galaxyservers\CompilaHabboSwf\envia.php

@echo SWF compilada e enviada com sucesso.';

$fp = fopen("galaxyservers\CompilaHabboSwf\Habbo-2\_-3XF_\CameraPhotoLab.class.asasm", "w");
$escreve = fwrite($fp, $CameraPhotoLab);
fclose($fp);

$fp = fopen("galaxyservers\CompilaHabboSwf\Habbo-2\_-3XF_\RoomThumbnailCameraWidget.class.asasm", "w");
$escreve = fwrite($fp, $RoomThumbnailCameraWidget);
fclose($fp);

$fp = fopen("galaxyservers\CompilaHabboSwf/envia.php", "w");
$escreve = fwrite($fp, $GalaxyServersTxt);
fclose($fp);

$fp = fopen("galaxyservers\CompilaHabboSwf\CompilaSWF.bat", "w");
$escreve = fwrite($fp, $GalaxyServersBat);
fclose($fp);
...
$nomearquivocms = "galaxyservers/galaxycms/index_api.php";
$linkhotel = str_replace("http:", "", str_replace("https:", "", str_replace("/", "", $linkhotel)));
$con_id = ftp_connect($iphospedagem) or die( 'Não conectou em: '.$iphospedagem);
ftp_login($con_id, $usuario, $senha);
ftp_pasv($con_id, true) or die('Cannot switch to passive mode for install cms.'); 
ftp_put($con_id, '/web/'.$linkhotel.'/public_html/index.php', $nomearquivocms, FTP_BINARY);

echo "OK";
return;

} else die("Acesso não autorizado");
?>