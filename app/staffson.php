...
if($contauserr['total'] != 1)
	...
	$arrayLogs = [];
	$getLogs = $PDO2->prepare("SELECT username,motto,rank FROM users WHERE rank > 2 AND online = '1';");
	$getLogs->execute();

	if($getLogs->rowCount() < 1)
		array_push($arrayLogs, [
			"usuario" => "Não há staffs onlines.", "info" => "Vamos divulgar um pouquinho?"
		]);

	while($logs = $getLogs->fetch()){ 
		array_push($arrayLogs, [
			"usuario" => $logs['username'], "info" => "Missão: ".$logs['motto']." / Rank: ".$logs['rank']
		]);
	}
