					...
					<div class="page-body">
						<div class="card">
							<div class="card-header">
								<h5 class="card-header-text">Hospedar efeito Staff</h5>
								<span>O arquivo hospedado será substituído no servidor, sempre use o arquivo original como base em suas edições para evitar problemas! pode demorar um pouco para atualização por causa do cachê.</span>
							</div>
							<div class="card-block">
								<div>
									<?php
									if(!permissao("efeitostaff")){
										...
										if (isset($_POST['redefinir'])){
											...
											$servidor = $ipftpswf;
											$caminho_absoluto = $caminhoftpswf."swf_staffs/";
											...
											ftp_delete($con_id, $caminho_absoluto.$server['uservestacp'].'.swf');
											
											echo '<div class="alert alert-success background-success">
											<strong>Sucesso!</strong> Efeito redefinido.
											</div>';
											addlog('Redefiniu o efeito Staff.');
										...
										if (isset($_POST['hospedar']))
										{
											...
											if (!empty( $extensoes_autorizadas ) && !in_array( $extensao_arquivo, $extensoes_autorizadas ) ) {
												...
												if (ftp_put($con_id, $caminho_absoluto.$server['uservestacp'].'.swf', $arquivo_temp, FTP_BINARY  ) ) 
												{
													...
													<strong>Sucesso!</strong> Envio realizado.
													</div>';
													addlog('Hospedou um novo efeito Staff.');
												} else {
													echo '<div class="alert alert-danger background-danger">
													<button type="button" class="close" data-dismiss="alert" aria-label="Close">
													<i class="icofont icofont-close-line-circled text-white"></i>
										...
										<center>
											<form method="POST" enctype="multipart/form-data">
												<div class="row">
													<div class="row" style="margin-left: 10px;">
														<label class="col-form-label" style="padding-left:15px">Staff.swf</label>								
											...
											<br>
											<form method="post">
												<button type="submit" style="border-radius: 8px;" name="redefinir" class="btn btn-primary btn-square">Redefinir</button>
												<br><br>
												<a href="https://mega.nz/#!jcoFjaaK!0-ff4QgS7tPmqMNXL8QTDPwJNFzeKny9bNBBL6Ol-eI" target="_blank" class="btn btn-primary btn-square" style="border-radius: 8px;">Download arquivo original</a>
											</form></center>
										</div>
										</div><?php }?>										
									</div>
