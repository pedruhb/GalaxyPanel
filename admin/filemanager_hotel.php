...
if(!empty($_GET['user'])){
	...
	$infoserver = $PDO->prepare("SELECT * FROM servidores WHERE nomeusuario = :user");
	$infoserver->bindParam(':user', $_GET['user']);
	$infoserver->execute();
		...
		<script>
			$(function() {
				$('#elfinder').elfinder(
				{
					...
					?>
						url : '/galaxyservers/filemanager/php/connector.minimal.php?user=<?php echo $server['uservestacp'];?>&password=<?php echo $server['senhavestacp'];?>&path=/&ip=<?= $iphospedagem;?>',
