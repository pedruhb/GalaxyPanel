								...
								<div class="card-block">		
									...
									if(isset($_POST['ativa'])){
										...
										if($continua){
											...
											$linkhotelsem = str_replace("https://", "", str_replace("http://", "", $hotel['linkhotel']));
											
                      if($_POST['swf'] == 1 || $_POST['swf'] == "1")
                        $nomepastacompilador = "CompilaHabboSwf";
                      else if($_POST['swf'] == 2 || $_POST['swf'] == "2")
                        $nomepastacompilador = "CompilaHabboSwf2";
#region Faz os bagulhos da Habbo.SWF
                      ...
                      if ( @ftp_put( $con_id, "./"."'.$hotel['uservestacp'].'".".swf", "C:\/xampp\htdocs\galaxyservers\/'.$nomepastacompilador.'\Habbo.swf", FTP_BINARY)) 
                      {
                        ...
                        @echo Compilando a SWF do '.$hotel['nomehotel'].' Hotel...
                        @echo ----------------------------------------------------
                        ...
                        @rabcasm Habbo-2/Habbo-2.main.asasm
                        @echo off
                        C:\xampp\php\php.exe C:\xampp\htdocs\galaxyservers\/'.$nomepastacompilador.'\envia.php
...
/// CameraPhotoLab.class.asasm
                        ...
                        pushstring          "'.$hotel['http'].'://'.$linkhotelsem.'/camera/index.php"
                        getlocal0
                        getproperty         QName(PrivateNamespace("_-4E0", "_-3XF:CameraPhotoLab#0"), "_-5dD")
...
//RoomThumbnailCameraWidget.class.asasm
                        ...
                        findpropstrict      QName(PackageNamespace("flash.net"), "URLRequest")
                        pushstring          "'.$hotel['http'].'://'.$linkhotelsem.'/camera/thumbnails/index.php?roomid="
                        getlocal            4
                        ...
                        $fp = fopen("..\galaxyservers\/".$nomepastacompilador."\Habbo-2\_-3XF_\CameraPhotoLab.class.asasm", "w");
                        $escreve = fwrite($fp, $CameraPhotoLab);
                        fclose($fp);

                        $fp = fopen("..\galaxyservers\/".$nomepastacompilador."\Habbo-2\_-3XF_\RoomThumbnailCameraWidget.class.asasm", "w");
                        $escreve = fwrite($fp, $RoomThumbnailCameraWidget);
                        fclose($fp);

                        $fp = fopen("..\galaxyservers\/".$nomepastacompilador."/envia.php", "w");
                        $escreve = fwrite($fp, $GalaxyServersTxt);
                        fclose($fp);

                        $fp = fopen("..\galaxyservers\/".$nomepastacompilador."\CompilaSWF.bat", "w");
                        $escreve = fwrite($fp, $GalaxyServersBat);
                        fclose($fp);

#endregion

                        exec('cd C:/xampp\htdocs\galaxyservers\/'.$nomepastacompilador.'\ && start Compilador.exe');
                        $_SESSION['timestamp-swf'] = time();	

                        loginsucess("SWF está sendo recompilada.");

                        addlog_admin($_SESSION['useradmin'], "Recompilou SWF do hotel \"".$hotel['uservestacp']."\".");
                      }
                    }
                    ?>						
                    <form method="POST">
                      <div class="form-group row">
                        ...
                        <div class="col-sm-10">                   
                          <select class="form-control" name="hotel_id">
                            <?php
                            $pegadados = $PDO->prepare("SELECT * FROM servidores WHERE ativo = 'Y';");
                            $pegadados->execute();
                            while($dados = $pegadados->fetch()){
                              ?>
                              <option value="<?= $dados['id'];?>"><?= $dados['uservestacp'];?> | <?= $dados['nomehotel'];?> | <?= $dados['linkhotel'];?></option>
                            <?php }?>
                          </select>
                        </div>
                      </div>  
                      ...
                      <center><button name="ativa" type="submit" class="btn btn-primary btn-square">Recompilar</button></center>
                    </form>
                  </div>
                </div>
              </div>
